...
class QueueStateHolderTest {
    ...
    private val onlineMusicRepository: OnlineMusicRepository = mockk(relaxed = true)

    private fun holder() = QueueStateHolder(musicRepository, onlineMusicRepository)
    ...
    @Test
    fun `playArtist uses YouTube Music top songs for an online artist`() = runTest {
        val artist = Artist(
            id = 123L,
            name = "Online Artist",
            songCount = 2,
            remoteBrowseId = "UC_online_artist",
        )
        val artistSongs = listOf(song1, song2)
        coEvery { onlineMusicRepository.getArtistProfile("UC_online_artist") } returns
            YouTubeArtistProfile(
                browseId = "UC_online_artist",
                name = artist.name,
        ...
        val cb = PlaybackSourceCallbacks(
            scope = this,
            playSongs = { songs, _, _, _ -> playedSongs = songs },
            showSheet = {},
        )
