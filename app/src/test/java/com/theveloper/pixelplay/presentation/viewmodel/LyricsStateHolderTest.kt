...
class LyricsStateHolderTest {
    ...
    @Test
    fun fetchLyricsForSong_usesStoredLyricsWithoutRemoteFetch() = runBlocking {
        ...
        val holderScope = CoroutineScope(SupervisorJob() + Dispatchers.Default)
        holder.initialize(holderScope, callback, state)
        ...
        val result = withTimeout(2_000) {
            holder.searchUiState.first { it is LyricsSearchUiState.Success }
        }
