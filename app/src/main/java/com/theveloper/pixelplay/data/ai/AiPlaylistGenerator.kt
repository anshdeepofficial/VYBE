...
class AiPlaylistGenerator @Inject constructor(
    ...
    private val userPrefsRepo: com.theveloper.pixelplay.data.preferences.UserPreferencesRepository,
    ...
    suspend fun generate(
        ...
        return try {
            val availableSongs = (allSongs + candidateSongs.orEmpty()).distinctBy { it.id }
            if (availableSongs.isEmpty()) {
                return Result.failure(
                    IllegalStateException("No songs are available yet. Refresh Daily Mix and try again.")
                )
            }
            ...
            val samplingPool = when {
                candidateSongs.isNullOrEmpty().not() -> candidateSongs
                    ...
                    if (rankedForPrompt.isNotEmpty()) rankedForPrompt else availableSongs
                }
            }
            ...
            val userDigest = digestGenerator.generateDigest(availableSongs, isSafe)
            ...
            // Online Daily Mix tracks are valid candidates even when the local library is empty.
