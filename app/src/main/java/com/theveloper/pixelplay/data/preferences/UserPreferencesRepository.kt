...
enum class SongIdentityMode {
    ...
    companion object {
        fun fromName(value: String?): SongIdentityMode =
...
enum class NotificationPlayerAction(val displayName: String) {
    REPEAT("Loop / Repeat"),
    SHUFFLE("Shuffle"),
    LIKE("Like"),
    CLOSE("Close player");
    ...
    companion object {
        fun fromName(value: String?, fallback: NotificationPlayerAction) =
...
class UserPreferencesRepository @Inject constructor(
    ...
    private object PreferencesKeys {
        ...
        val IMMERSIVE_ARTWORK_ENABLED = booleanPreferencesKey("immersive_artwork_enabled")
        ...
        val NOTIFICATION_ACTION_ONE = stringPreferencesKey("notification_action_one")
        val NOTIFICATION_ACTION_TWO = stringPreferencesKey("notification_action_two")
        ...
        val SAVED_PLAYBACK_QUEUES = stringPreferencesKey("saved_playback_queues_v1")
        ...
        val SONG_IDENTITY_MODE = stringPreferencesKey("song_identity_mode")
    ...
    suspend fun updatePreferredGenres(genres: Set<String>) {
        dataStore.edit { it[PreferencesKeys.PREFERRED_GENRES] = genres }
    }
    ...
    suspend fun setSearchHistoryEnabled(enabled: Boolean) {
        dataStore.edit { it[PreferencesKeys.SEARCH_HISTORY_ENABLED] = enabled }
    }

    val userRegionFlow: kotlinx.coroutines.flow.Flow<String> =
        pref { it[PreferencesKeys.USER_REGION] ?: "Global" }

    suspend fun setUserRegion(region: String) {
        dataStore.edit { it[PreferencesKeys.USER_REGION] = region }
    }
    ...
    val youtubeMusicSignInPromptShownFlow: Flow<Boolean> =
        pref { it[PreferencesKeys.YOUTUBE_MUSIC_SIGN_IN_PROMPT_SHOWN] ?: false }

    suspend fun setYouTubeMusicSignInPromptShown(shown: Boolean) {
    ...
    val notificationActionOneFlow: Flow<NotificationPlayerAction> = pref {
        NotificationPlayerAction.fromName(
            it[PreferencesKeys.NOTIFICATION_ACTION_ONE],
            NotificationPlayerAction.REPEAT,
        )
    }

    val notificationActionTwoFlow: Flow<NotificationPlayerAction> = pref {
        NotificationPlayerAction.fromName(
            it[PreferencesKeys.NOTIFICATION_ACTION_TWO],
            NotificationPlayerAction.SHUFFLE,
        )
    }

    suspend fun setNotificationActionOne(action: NotificationPlayerAction) {
        dataStore.edit { it[PreferencesKeys.NOTIFICATION_ACTION_ONE] = action.name }
    }

    suspend fun setNotificationActionTwo(action: NotificationPlayerAction) {
        dataStore.edit { it[PreferencesKeys.NOTIFICATION_ACTION_TWO] = action.name }
    }
    ...
    val savedPlaybackQueuesFlow: Flow<List<SavedPlaybackQueue>> =
        pref { preferences ->
            preferences[PreferencesKeys.SAVED_PLAYBACK_QUEUES]?.let { raw ->
                runCatching { json.decodeFromString<List<SavedPlaybackQueue>>(raw) }.getOrNull()
            }.orEmpty().sortedByDescending { it.savedAtEpochMs }
        }

    suspend fun savePlaybackQueue(queue: SavedPlaybackQueue) {
        dataStore.edit { preferences ->
            val existing = preferences[PreferencesKeys.SAVED_PLAYBACK_QUEUES]?.let { raw ->
                runCatching { json.decodeFromString<List<SavedPlaybackQueue>>(raw) }.getOrNull()
            }.orEmpty()
            val updated = (existing.filterNot { it.id == queue.id } + queue)
                .sortedByDescending { it.savedAtEpochMs }
                .take(MAX_SAVED_PLAYBACK_QUEUES)
            preferences[PreferencesKeys.SAVED_PLAYBACK_QUEUES] = json.encodeToString(updated)
        }
    }

    suspend fun deleteSavedPlaybackQueue(queueId: String) {
        dataStore.edit { preferences ->
            val existing = preferences[PreferencesKeys.SAVED_PLAYBACK_QUEUES]?.let { raw ->
                runCatching { json.decodeFromString<List<SavedPlaybackQueue>>(raw) }.getOrNull()
            }.orEmpty()
            preferences[PreferencesKeys.SAVED_PLAYBACK_QUEUES] =
                json.encodeToString(existing.filterNot { it.id == queueId })
        }
    }
...
suspend fun markDirectoryRulesVersionApplied(version: Int) {
    ...
    val songIdentityModeFlow: Flow<SongIdentityMode> =
        dataStore.data.map { preferences ->
            SongIdentityMode.fromName(preferences[PreferencesKeys.SONG_IDENTITY_MODE])
        }.distinctUntilChanged()

    suspend fun setSongIdentityMode(mode: SongIdentityMode) {
        dataStore.edit { preferences ->
            preferences[PreferencesKeys.SONG_IDENTITY_MODE] = mode.name
            preferences[PreferencesKeys.LAST_SYNC_TIMESTAMP] = 0L
        }
    }
    ...
    fun backupRevisionFlow(): Flow<Int> = dataStore.data
        .map { preferences -> preferences.asMap().hashCode() }
        .distinctUntilChanged()
