...
suspend fun loadArtworkBytesViaCoil(context: Context, uri: Uri): ByteArray? {
    ...
    return runCatching {
        ...
        Timber.tag("MusicService_VYBE").w(error, "Artwork read failed via Coil for uri=%s", uri)
        null
    }
}
...
class MusicService : MediaLibraryService() {
    ...
    private var notificationActionOne = NotificationPlayerAction.REPEAT
    private var notificationActionTwo = NotificationPlayerAction.SHUFFLE
            ...
            override fun onChange(selfChange: Boolean) {
                ...
                if (streamVolume == 0) {
                    ...
                    if (!player.isPlaying && player.currentMediaItem != null) {
                        player.play()
                        Timber.tag(TAG).d("pauseOnVolumeZero: resumed because system media volume increased")
                    }
                }
            }
        }
    ...
    companion object {
        private const val TAG = "MusicService_VYBE"
        const val NOTIFICATION_ID = 101
        private const val TEMPORARY_NOTIFICATION_ID = 102
    ...
    private fun syncLocalListeningStatsFromPlayer(
        ...
        val fallbackDurationMs = mediaItem.mediaMetadata.extras
            ...
            ?: 0L
        val statsTrack = mediaMapper.resolveSongFromMediaItem(mediaItem)
            ?.let(com.theveloper.pixelplay.data.stats.PlaybackStatsRepository.PlaybackTrackSnapshot::fromSong)
    ...
    override fun onCreate() {
        ...
        serviceScope.launch {
            userPreferencesRepository.pauseOnVolumeZeroFlow.collect { enabled ->
                ...
                if (!enabled) pausedByVolumeZero = false
            }
        }
        ...
        serviceScope.launch {
            userPreferencesRepository.notificationActionOneFlow.collect { action ->
                notificationActionOne = action
                mediaSession?.let { refreshMediaSessionUi(it, force = true) }
            }
        }
        serviceScope.launch {
            userPreferencesRepository.notificationActionTwoFlow.collect { action ->
                notificationActionTwo = action
                mediaSession?.let { refreshMediaSessionUi(it, force = true) }
            }
        }
    ...
    private fun cancelTemporaryForegroundNotification() {
        (getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager)
            .cancel(TEMPORARY_NOTIFICATION_ID)
    }
    ...
    private fun buildMediaButtonPreferencesSignature(session: MediaSession): String {
        ...
        return buildString {
            ...
            append('|')
            append(notificationActionOne.name)
            append('|')
            append(notificationActionTwo.name)
        }
    }
    ...
    private fun buildMediaButtonPreferences(session: MediaSession): List<CommandButton> {
        ...
        val buttonsByAction = mapOf(
            NotificationPlayerAction.REPEAT to repeatButton,
            NotificationPlayerAction.SHUFFLE to shuffleButton,
            NotificationPlayerAction.LIKE to likeButton,
            NotificationPlayerAction.CLOSE to closeButton,
        )
        val preferredActions = listOf(notificationActionOne, notificationActionTwo).distinct()
        val remainingActions = NotificationPlayerAction.entries.filterNot(preferredActions::contains)
        return (preferredActions + remainingActions).mapNotNull(buttonsByAction::get)
    }
