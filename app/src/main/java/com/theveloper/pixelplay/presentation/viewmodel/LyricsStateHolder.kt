...
class LyricsTranslationCallbacks(
    ...
    val transliterateToEnglishScript: suspend (String) -> Result<String>,
...
class LyricsStateHolder @Inject constructor(
    ...
    fun translateLyricsViaAi(currentSong: Song, lyricsObj: Lyrics?, cb: LyricsTranslationCallbacks) {
        // Streamed YouTube Music songs use non-numeric IDs. Translation must still run and update
        // the active lyrics even when there is no local MediaStore row to write to.
        val localSongId = currentSong.id.toLongOrNull()
            ...
            result.onSuccess { translatedText ->
                ...
                if (translatedText.isNotBlank() && !lyricsObj?.synced.isNullOrEmpty()) {
                    ...
                    if (validation is LyricsImportValidationResult.Valid) {
                        ...
                        val updatedSong = currentSong.withPersistedLyrics(
                            validated.sanitizedContent,
                            refreshedAlbumArtUri,
                        )
                        _searchUiState.value = LyricsSearchUiState.Success(validated.parsedLyrics)
                        _songUpdates.emit(updatedSong to validated.parsedLyrics)
                        _messageEvents.emit(cb.getString(R.string.lyrics_translate_success))
                    } else {
    ...
    fun transliterateLyricsToEnglishScript(
        ...
        scope?.launch {
            _messageEvents.emit(cb.getString(R.string.lyrics_transliterate_progress))
            val rawLyrics = withContext(Dispatchers.IO) {
                currentSong.lyrics?.takeIf { it.isNotBlank() }
                    ?: readLocalLyricsFile(currentSong)
                    ?: readEmbeddedLyricsFromFile(currentSong)
            ...
            cb.transliterateToEnglishScript(rawLyrics).onSuccess { generated ->
                ...
                if (displayedLyrics == null) {
                    _messageEvents.emit(cb.getErrorString("The AI response did not preserve the lyric lines"))
                } else {
                    _searchUiState.value = LyricsSearchUiState.Success(displayedLyrics)
                    _songUpdates.emit(currentSong to displayedLyrics)
