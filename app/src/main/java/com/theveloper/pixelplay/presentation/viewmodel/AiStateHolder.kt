...
class AiStateHolder @Inject constructor(
    ...
    fun generateAiPlaylist(
        ...
        scope.launch {
            if (prompt.isBlank()) {
                _aiError.value = context.getString(R.string.ai_state_prompt_empty)
                return@launch
            }
            val localSongs = allSongsProvider?.invoke() ?: emptyList()
            val allSongs = (
                localSongs +
                    dailyMixStateHolder.dailyMixSongs.value +
                    dailyMixStateHolder.yourMixSongs.value +
                    dailyMixStateHolder.quickPickSongs.value +
                    dailyMixStateHolder.latestReleaseSongs.value
                ).distinctBy { it.id }
...
$lyricsText
    ...
    suspend fun transliterateLyricsToEnglishScript(lyricsText: String): Result<String> {
        return try {
            val hasTimestamps = Regex("\\[\\d{1,2}:\\d{2}(?:[.:]\\d{1,3})?]").containsMatchIn(lyricsText)
...
""".trimIndent()
                ...
                """
- Output exactly ONE phonetic English-script line for every input lyric line, in the same order.
- Do not repeat the original line and do not add timestamps.
""".trimIndent()
            }
            val prompt = """
<task>Transliterate these lyrics phonetically into Latin/English script. Preserve how the words sound; do NOT translate their meaning.</task>

<rules>
$formatRules
- Never add explanations, labels, numbering, headers, or markdown.
- Preserve blank lines and line order.
- Keep names and words that already use Latin script unchanged.
</rules>
