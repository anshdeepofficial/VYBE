...
data class PlaylistUiState(
    ...
    val youtubeSyncState: YouTubeSyncState = YouTubeSyncState.IDLE,
...
class PlaylistViewModel @Inject constructor(
    ...
    init {
        ...
        observeYouTubePlaylistSync()
    }

    private fun observeYouTubePlaylistSync() {
        viewModelScope.launch {
            youTubeAccountManager.syncEnabledPlaylistIds.collect { ids ->
                _uiState.update { it.copy(youtubeSyncEnabledPlaylistIds = ids) }
            }
        }
        viewModelScope.launch {
            youTubeAccountManager.isLoggedInFlow.collect { connected ->
                _uiState.update { it.copy(isYouTubeMusicConnected = connected) }
            }
        }
        viewModelScope.launch {
            youTubeAccountManager.syncStateFlow.collect { state ->
                _uiState.update { it.copy(youtubeSyncState = state) }
            }
        }
    }
    ...
    fun deletePlaylist(playlistId: String) {
        ...
        viewModelScope.launch {
            val playlist = _uiState.value.playlists.firstOrNull { it.id == playlistId }
                ?: _uiState.value.currentPlaylistDetails?.takeIf { it.id == playlistId }
            if (playlist != null) youTubeAccountManager.onLocalPlaylistDeleting(playlist)
            playlistPreferencesRepository.deletePlaylist(playlistId)
        }
    }

    fun setYouTubeMusicSyncEnabled(playlistId: String, enabled: Boolean) {
        if (isFolderPlaylistId(playlistId)) return
        youTubeAccountManager.setPlaylistSyncEnabled(playlistId, enabled)
    }
    ...
    fun renamePlaylist(playlistId: String, newName: String) {
        ...
        viewModelScope.launch {
            ...
            youTubeAccountManager.requestPlaylistSync(playlistId)
            ...
            playlistIds.forEach { playlistId ->
                if (!isFolderPlaylistId(playlistId)) {
                    _uiState.value.playlists.firstOrNull { it.id == playlistId }
                        ?.let { youTubeAccountManager.onLocalPlaylistDeleting(it) }
                    playlistPreferencesRepository.deletePlaylist(playlistId)
                }
            }
        }
    ...
    fun exportPlaylistsAsM3u(playlistIds: List<String>) {
        ...
        viewModelScope.launch {
            try {
                ...
                val exportDir = File(musicDir, "VYBE Exports")
                if (!exportDir.exists()) {
                    exportDir.mkdirs()
                }
