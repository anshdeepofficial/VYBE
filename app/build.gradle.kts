...
val releaseStoreFilePath = keystoreProperties.getProperty("storeFile").orEmpty().trim()
val releaseStorePassword = keystoreProperties.getProperty("storePassword").orEmpty()
val releaseKeyAlias = keystoreProperties.getProperty("keyAlias").orEmpty().trim()
val releaseKeyPassword = keystoreProperties.getProperty("keyPassword").orEmpty()
val releaseStoreFile = releaseStoreFilePath.takeIf(String::isNotBlank)?.let(rootProject::file)
...
android {
    ...
    defaultConfig {
        applicationId = "com.vybe.musicplayer"
        // Android 8.0+; newer platform APIs are guarded at their call sites.
        minSdk = 26
        targetSdk = 37
        ...
        val googleDriveWebClientId = localProperties.getProperty("GOOGLE_DRIVE_WEB_CLIENT_ID")
            ?.trim()
            .orEmpty()
        ...
        val vybeGitHubOwner = localProperties.getProperty("VYBE_GITHUB_OWNER")
            ...
            ?: "anshdeepofficial"
        val vybeGitHubRepo = localProperties.getProperty("VYBE_GITHUB_REPO")
            ?.trim()
            ?.takeIf(String::isNotBlank)
            ?: "VYBE"
